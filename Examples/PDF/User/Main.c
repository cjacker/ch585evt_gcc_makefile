...
void Uart_Init(void)
{
    ...
    GPIOA_ModeCfg(GPIO_Pin_15, GPIO_ModeIN_PU);      // RXD-Configure pull-up input
    GPIOA_ModeCfg(GPIO_Pin_14, GPIO_ModeOut_PP_5mA); // TXD-Configure push-pull output, paying attention to first allowing the IO port to output a high level
    UART0_DefInit();
}
...
int main()
{
    ...
    SetSysClock(CLK_SOURCE_HSE_PLL_62_4MHz);              // The system running clock is configured to 62_4MHz
