...
static CONST uint8_t attDeviceName[GAP_DEVICE_NAME_LEN] = "PICC BLE";
...
static void hidEmuStateCB(gapRole_States_t newState, gapRoleEvent_t *pEvent)
{
    switch(newState & GAPROLE_STATE_ADV_MASK)
    {
        ...
        {
            GAPRole_GetParameter(GAPROLE_BD_ADDR, hid_static_addr);
            hid_static_addr[5] |= 0xc0;
            GAP_ConfigDeviceAddr(ADDRTYPE_STATIC, hid_static_addr);
            PRINT("Initialized..\n");
            PRINT("hid_static_addr: %x %x %x %x %x %x\n",
                    hid_static_addr[0], hid_static_addr[1], hid_static_addr[2],
                    hid_static_addr[3], hid_static_addr[4], hid_static_addr[5]);
            hid_nfca_m1_ndef_init();
        }
        break;
