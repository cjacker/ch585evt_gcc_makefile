...
#define TMOS_TASK_STOP_RELOAD_TIME      0xffffff    /* 防止内存碎片，使用reload task代替普通task。会一直占用内存，不释放，保证程序稳定运行，时间为TMOS_TASK_STOP_RELOAD_TIME * 0.625us。 */
...
static void RF_2G4StatusCallBack(uint8_t sta, uint8_t crc, uint8_t *rxBuf)
{
    ...
    switch(sta)
    {
        ...
        case TX_MODE_TX_FAIL:
            tx_end_flag = TRUE;
            tmos_start_reload_task(lwns_phyoutput_taskid, LWNS_PHY_OUTPUT_FINISH_EVT, TMOS_TASK_STOP_RELOAD_TIME);      /* 变相停止任务，占用内存不释放 */
...
static BOOL ble_phy_output(uint8_t *dataptr, uint8_t len)
{
    ...
    pMsg = tmos_msg_allocate((((len + 1 + 15) & 0xf0) + 1 + 1)); /* 校验位1字节加上后再进行16字节对齐，存储发送长度+1，真实数据长度+1 */
  #else
...
static uint16_t lwns_adapter_ProcessEvent(uint8_t task_id, uint16_t events)
{
    ...
    if(events & LWNS_PHY_RX_OPEN_EVT)   /* 重新打开接收事件 */
    {
        RF_Shut();
        RF_Rx(NULL, 0, USER_RF_RX_TX_TYPE, USER_RF_RX_TX_TYPE); /* 重新打开接收 */
        return (events ^ LWNS_PHY_RX_OPEN_EVT);
    }
    if(events & SYS_EVENT_MSG)  /* 处理打开接收后，再处理数据 */
    {
        uint8_t *pMsg;
...
static uint16_t lwns_phyoutput_ProcessEvent(uint8_t task_id, uint16_t events)
{
    ...
    if(events & LWNS_PHY_OUTPUT_PREPARE_EVT)    /* 准备发送工作 */
    {
        uint8_t rand_delay;
        ...
        PRINTF("rand send:%d\n", rand_delay);
        tmos_start_reload_task(lwns_phyoutput_taskid, LWNS_PHY_OUTPUT_FINISH_EVT, rand_delay);  /* 随机延迟后立刻发送 */
    ...
    if(events & LWNS_PHY_OUTPUT_FINISH_EVT)     /* 发送完成任务 */
    {
        if(ble_phy_channelmap_send_seq < sizeof(ble_phy_channelmap))
        {
            ...
            {
                /* 只需要在开始发送的时候停任务，发送期间，停止继续切换接收通道，变相停止任务，占用内存不释放 */
            ...
            tmos_start_reload_task(lwns_phyoutput_taskid, LWNS_PHY_OUTPUT_FINISH_EVT, MS1_TO_SYSTEM_TIME(LWNS_PHY_OUTPUT_TIMEOUT_MS)); //开始发送超时计数，防止有意外打断发送，导致无法继续发送
